...
def generate_sudoku():
    ...
    for r in range(9):
        for c in range(9):
            entries[r][c].config(state="normal")
            entries[r][c].delete(0, tk.END)

            if puzzle[r][c] != 0:
                entries[r][c].insert(0, str(puzzle[r][c]))
                entries[r][c].config(state="disabled", disabledforeground="black")
                original_cells[r][c] = True
            else:
                original_cells[r][c] = False
...
def solve_sudoku():
    for r in range(9):
        for c in range(9):
            ...
            if original_cells[r][c]:
                entries[r][c].config(state="disabled", disabledforeground="black")
            else:
                entries[r][c].config(state="disabled", disabledforeground="blue")
