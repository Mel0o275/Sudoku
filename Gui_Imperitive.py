...
for i in range(9):
    for j in range(9):
        cell_frame = tk.Frame(frame, highlightbackground="black", highlightthickness=1)
        cell_frame.grid(row=i, column=j, sticky="nsew")
        entry = tk.Entry(cell_frame, width=3, font=('Helvetica', 18), justify='center')
...
def generate_sudoku():
    ...
    global question_code, solution_code
    question_code, solution_code = board.generateQuestionBoardCode(param)

    for i in range(9):
        for j in range(9):
            num = int(question_code[i*9 + j])
            ...
            if num != 0:
                ...
                entries[i][j].config(state='normal', disabledforeground='black')
                original_cells[i][j] = False 

generate_button = tk.Button(root, text="Generate Sudoku", command=generate_sudoku)
generate_button.pack(pady=5)

def solve_sudoku():
    solved_board = Board(question_code).solveForCode()
    for i in range(9):
        for j in range(9):
            entries[i][j].config(state='normal')
            entries[i][j].delete(0, tk.END)
            entries[i][j].insert(0, solved_board[i*9 + j])
            if original_cells[i][j]:
                entries[i][j].config(disabledforeground='black')
            else:
                entries[i][j].config(disabledforeground='blue')
            entries[i][j].config(state='disabled')
